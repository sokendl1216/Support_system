...
async def test_knowledge_base_operations(rag_system: RAGSystem):
    """知識ベース操作テスト"""
    logger.info("=== 知識ベース操作テスト開始 ===")
    ...
    try:
        ...
        if total_docs > 0:
            logger.info("✅ 知識ベース操作テスト成功")
        else:
            logger.warning("⚠️ 文書が取り込まれていません")
            
    except Exception as e:
        logger.error(f"❌ 知識ベース操作テスト失敗: {e}")
        raise
        ...
        try:
            ...
            for mode in ['interactive', 'autonomous']:
                ...
                if results:
                    ...
                    logger.info(f"  トップ結果類似度: {similarity:.3f}")
...
async def test_mode_specific_optimization():
    ...
    try:
        ...
        if interactive_results and autonomous_results:
            ...
            # 設定の違いを確認
            interactive_config = config.get('mode_optimization', {}).get('interactive', {})
            autonomous_config = config.get('mode_optimization', {}).get('autonomous', {})
            
            logger.info(f"対話型設定: {interactive_config}")
            logger.info(f"自律型設定: {autonomous_config}")
...
async def main():
    ...
    try:
        ...
        # 2. 知識ベース操作テスト
        await test_knowledge_base_operations(rag_system)
        ...
        logger.info("📋 Task 3-3 (RAG検索システム実装) の動作確認完了")
        logger.info("📋 Task 3-3a (進行モード別知識ベース最適化) の準備完了")
